...
describe('callback route', () => {
  ...
  it('exchanges code, sets session cookie, and redirects to stored redirect on success', async () => {
    ...
    const res = await app.request('/auth/callback?code=abc&state=expected-state', {
      headers: {
        cookie:
          'oauth_state=expected-state; oauth_nonce=expected-nonce; oauth_redirect=https://frontend.example.com/dashboard',
      },
      redirect: 'manual',
    });

    expect(res.status).toBe(302);
    expect(res.headers.get('location')).toBe('https://frontend.example.com/dashboard');
