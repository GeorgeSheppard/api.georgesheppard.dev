...
describe('GET /api/reextract-recurring-books', () => {
  ...
  test('should return 200 with empty summary when there are no unprocessed images', async () => {
    const response = await app.request(
      new Request('http://localhost/api/reextract-recurring-books', {
        method: 'GET',
        headers: {
  ...
  test('should extract and store books for an unprocessed image on a recurring user', async ({
    dbClient,
  }) => {
    ...
    const [updatedImage] = await dbClient.db.select().from(images).where(eq(images.id, image.id));

    expect(updatedImage.extractedBooks).toEqual([{ title: 'Dune', author: 'Frank Herbert' }]);
    expect(updatedImage.processedUtc).toBeInstanceOf(Date);
  });

  test('should skip images that already have extracted books', async ({ dbClient }) => {
    const [request] = await dbClient.db
      .insert(requests)
      .values({
        frequency: 'M',
    ...
    await dbClient.db.insert(images).values({
      ...
      image: Buffer.from('fake-image-data'),
      contentType: 'image/jpeg',
      extractedBooks: [{ title: 'Already processed', author: null }],
  ...
  test('should not process images for non-recurring requests', async ({ dbClient }) => {
