...
const BodySchema = z.object({
  ...
  recurring: z.enum(['true', 'false']).transform(v => v === 'true').optional(),
});
...
export function registerAddEmailRoute(app: OpenAPIHono) {
  app.openapi(
    ...
    async (c) => {
      ...
      const recommendationsAlreadyDone = !!(recommendation[0].processedUtc && recommendation[0].recommendations);
