...
# on the mac mini (e.g. ~/Documents/root). Regenerates .env from Infisical (Machine Identity /
# Universal Auth, no login needed), then pulls and (re)starts the stack. `docker compose up -d`
# only recreates containers whose config actually changed, so this is safe to run unconditionally.
#
